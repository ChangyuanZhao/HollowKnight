...
class Hp_getter():
    ...
    def get_souls(self):
        base_address = self.UnityPlayer + 0xFDD89C
        offset_address = ctypes.c_long()
        offset_list = [0x38, 0x54, 0xC, 0xA8, 0x120]
    ...
    def get_self_hp(self):
        ...
        offset_address = ctypes.c_long()
        offset_list = [0x3B4, 0x24, 0x34, 0x40, 0x10C, 0xE4]
        self.kernal32.ReadProcessMemory(int(self.process_handle), base_address, ctypes.byref(offset_address), 4, None)
        for offset in offset_list:
            self.kernal32.ReadProcessMemory(int(self.process_handle), offset_address.value + offset,
                                            ctypes.byref(offset_address), 4, None)
    ...
    def get_boss_hp(self):
        base_address = self.mono + 0x1F72C4
        offset_address = ctypes.c_long()
        offset_list = [0x54, 0x85C, 0x10, 0x20, 0x10, 0xAC]
        self.kernal32.ReadProcessMemory(int(self.process_handle), base_address, ctypes.byref(offset_address), 4, None)
        for offset in offset_list:
            self.kernal32.ReadProcessMemory(int(self.process_handle), offset_address.value + offset,
                                            ctypes.byref(offset_address), 4, None)
    ...
    def get_play_location(self):
        x = ctypes.c_long()
        x.value += self.UnityPlayer + 0xFEF994
        offset_list = [0x50, 0xA0, 0x8, 0x80, 0xC]
        ...
        offset_list = [0x24, 0x104, 0x98, 0x2C, 0xC]
    ...
    def get_hornet_location(self):
        base_address = self.UnityPlayer + 0xFDA300
        x = ctypes.c_long()
        offset_list = [0xE8, 0x48, 0x19C, 0x94, 0x5C, 0xC]
        ...
        base_address = self.UnityPlayer + 0xFEEBE4
        y = ctypes.c_long()
        offset_list = [0x24, 0x19C, 0x94, 0x7C, 0x5C, 0x28]
